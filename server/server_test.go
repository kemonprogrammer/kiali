...
func TestRootContextPath(t *testing.T) {
	...
	server, _ := NewServer(t.Context(), cpm, cf, cache, conf, nil, nil, nil, nil, filetest.StaticAssetDir(t))
	server.Start()
...
func TestAnonymousMode(t *testing.T) {
	...
	server, _ := NewServer(t.Context(), cpm, cf, cache, conf, &prom, nil, nil, nil, filetest.StaticAssetDir(t))
	server.Start()
	t.Logf("Started test http server: %v", serverURL)
	defer func() {
		server.Stop()
...
func TestSecureComm(t *testing.T) {
	...
	cache := cache.NewTestingCacheWithFactory(t, cf, *conf)
	prom := prometheustest.FakeClient{}
	server, err := NewServer(t.Context(), cpm, cf, cache, conf, &prom, nil, nil, nil, filetest.StaticAssetDir(t))
	require.NoError(err)
	server.Start()
	t.Logf("Started test http server: %v", serverURL)
	defer func() {
